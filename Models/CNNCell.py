...
def conv_layer_bn_1_dimension(layer_name, prev_layer, out_channel, shape=3, strides=1, padding='SAME', is_bn_training=True,
                              epsilon=1e-3, decay=0.90, stddev=0.05, wl=0.):
    """
       使用给定的参数作为输入创建卷积层
        :param layer_name : String
    ...
    with tf.name_scope(layer_name):
        with tf.name_scope('weights'):
            c_weights = variable_with_weight_2loss([shape, 1, in_channels, out_channels], stddev, wl)
            variable_summaries('conv_weights', c_weights)

        with tf.name_scope('linear_output'):
            layer = tf.nn.conv2d(prev_layer, c_weights, strides=[1, strides, 1, 1], padding=padding)
            tf.summary.histogram('linear', layer)
